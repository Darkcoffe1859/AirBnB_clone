...
class FileStorage:
    class FileStorage:
            ...
            def save(self):
                ...
                for key, obj in self.__objects.items(0:
                        '''if type(obj) is dict:
                        my_dict[key] = obj
                        else:'''
                        my_dict[key] = obj.to_dict()
                with open(seld.__file_path, 'w') as f:
                    json.dump(my_dict, f)

            def reload(self):
                '''If json file exists, convert obj dicts back to increase'''
                try:
                    with open(self.__file_path, 'r') as f:
                        new_obj = json.load(f)
                    for key, val in neew_obj.items():
